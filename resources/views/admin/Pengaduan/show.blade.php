...
@section('header')
    <a href="{{ url()->previous() }}" class="text-primary">Data Pengaduan</a>
                    ...
                    <table class="table">
                        <tbody>
                            ...
                            <tr>
                                <th>Foto</th>
                                <td>:</td>
                                <td><img src="{{url('storage/'.$pengaduan->foto)}}" class="img-fluid" alt="{{ $pengaduan->judul_laporan }}"></td>
                            </tr>
                            <tr>
                                <th>Judul Laporan</th>
                                <td>:</td>
    ...
    $(document).on('click', '.pengaduan', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "DELETE",
                    url: '{{ route('pengaduan.destroy', $pengaduan->id_pengaduan) }}',
                    data: {
                        "_token": "{{ csrf_token() }}"
                    },
                    success: function (response) {
                        if (response == 'success') {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    location.href = "{{ route('call.filterPengaduan', ['status' => '0']) }}";
                                }else{
                                    location.href = "{{ route('call.filterPengaduan', ['status' => '0']) }}";
                                }
                            });
